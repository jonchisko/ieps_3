...
def process_string(s, filename):
    ...
    vocab = [word for word in a if word.lower() not in stop_words_slovene]
...
def repetitions2(vocab, a):
    vocab = set(vocab)
    indexes = dict()
    for i in range(len(a)):
        print(f"            {i / len(a)} done...")
        if a[i] in vocab:
            word = a[i].lower()
            if word in indexes:
                indexes[word].append(str(i))
            else:
                indexes[word] = [str(i)]
    return indexes
    ...
    for domain in directories:
        ...
        for filename in get_files_in_directory(domain):
            ...
            for word in indexes:
                ...
                frequency.append(len(indexes[word]))
                #indeksi.append(" ".join(indexes[word]["indexes"]))
                indeksi.append(" ".join(indexes[word]))
